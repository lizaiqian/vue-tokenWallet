<template>
  <div>
    ...
    <div>
      <input type="text" v-model="childAccountName">
    ...
    <div>
      <select name="accountIndex" id="accountIndex" v-model="accountIndex">
        ...
        <option v-for="account in childAccounts">{{account.index}}</option>
      </select>
  ...
  export default {
    ...
    methods: {
      ...
      async handleTransaction() {
        ...
        if(balance < this.transactionEth * this.ethUnit) {
          throw Error('余额不足');
          return;
        }
      ...
      async createChildAccount() {
        this.childAccounts.push({
          name: this.childAccountName,
          index: this.childAccounts.length + 1
        });
      },
